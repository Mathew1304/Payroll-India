-- ============================================================================
-- FIX: Add missing allowance and comprehensive employee fields
-- ============================================================================
-- This migration adds columns that are referenced in the application 
-- but missing from the database, specifically the accommodation_allowance
-- column that's causing the current error.
--
-- INSTRUCTIONS:
...
BEGIN
  -- ========================================================================
  -- CRITICAL: Allowances (Required for Qatar/Saudi payroll)
  -- ========================================================================
  ...
  ELSE
    RAISE NOTICE 'accommodation_allowance column already exists';
  END IF;
  ...
  ELSE
    RAISE NOTICE 'transportation_allowance column already exists';
  END IF;
  ...
  ELSE
    RAISE NOTICE 'food_allowance column already exists';
  END IF;
  ...
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                 WHERE table_name = 'employees' 
                 AND column_name = 'mother_name') THEN
    ALTER TABLE employees ADD COLUMN mother_name text;
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                 WHERE table_name = 'employees' 
                 AND column_name = 'spouse_name') THEN
  ...
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                 WHERE table_name = 'employees' 
                 AND column_name = 'emergency_contact_alternate') THEN
  ...
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                 WHERE table_name = 'employees' 
                 AND column_name = 'highest_qualification') THEN
  ...
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                 WHERE table_name = 'employees' 
                 AND column_name = 'year_of_completion') THEN
  ...
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                 WHERE table_name = 'employees' 
                 AND column_name = 'additional_qualifications') THEN
  ...
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                 WHERE table_name = 'employees' 
                 AND column_name = 'total_experience_years') THEN
  ...
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                 WHERE table_name = 'employees' 
                 AND column_name = 'languages_known') THEN
  ...
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                 WHERE table_name = 'employees' 
                 AND column_name = 'visa_number') THEN
    ALTER TABLE employees ADD COLUMN visa_number text;
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                 WHERE table_name = 'employees' 
                 AND column_name = 'visa_expiry') THEN
