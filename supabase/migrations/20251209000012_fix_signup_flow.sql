/*
  # Fix Signup Flow with RPC
  
  ## Problem
  Client-side inserts into `organizations`, `organization_members`, `user_profiles`, and `organization_subscriptions` 
  are prone to RLS errors and complexity during sign-up, especially if email confirmation is involved 
  or RLS policies are strict.
  
  ## Solution
  Create a `SECURITY DEFINER` function `create_new_organization_flow` that handles the entire transaction atomically.
  This bypasses RLS checks for the insertion steps and ensures data integrity.
*/
